...
class LASLIB_DLL LASreader {
 public:
  ...
  virtual BOOL has_layers() const {
    return FALSE;
  };

  void set_index(LASindex* index);
  inline LASindex* get_index() const {
    return index;
  };
  ...
  inline LAStransform* get_transform() const {
    return transform;
  };
  void set_ignore(LASignore* ignore);
  inline LASignore* get_ignore() const {
    return ignore;
  };

  inline U32 get_inside() const {
    return inside;
  };
  virtual BOOL inside_none();
  virtual BOOL inside_tile(const F32 ll_x, const F32 ll_y, const F32 size);
  inline F32 get_t_ll_x() const {
    return t_ll_x;
  };
  inline F32 get_t_ll_y() const {
    return t_ll_y;
  };
  inline F32 get_t_size() const {
    return t_size;
  };
  virtual BOOL inside_circle(const F64 center_x, const F64 center_y, const F64 radius);
  inline F64 get_c_center_x() const {
    return c_center_x;
  };
  inline F64 get_c_center_y() const {
    return c_center_y;
  };
  inline F64 get_c_radius() const {
    return c_radius;
  };
  virtual BOOL inside_rectangle(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y);
  inline F64 get_r_min_x() const {
    return r_min_x;
  };
  inline F64 get_r_min_y() const {
    return r_min_y;
  };
  inline F64 get_r_max_x() const {
    return r_max_x;
  };
  inline F64 get_r_max_y() const {
    return r_max_y;
  };
  virtual BOOL inside_copc_depth(const U8 mode, const I32 depth, const F32 resolution);
  inline I32 get_copc_depth() const {
    return copc_depth;
  };
  inline F32 get_copc_resolution() const {
    return copc_resolution;
  };

  virtual BOOL seek(const I64 p_index) = 0;
  BOOL read_point() {
    return (this->*read_simple)();
  };

  inline BOOL ignore_point() {
    return (ignore ? ignore->ignore(&point) : FALSE);
  };
  ...
  inline F64 get_min_x() const {
    return header.min_x;
  };
  inline F64 get_min_y() const {
    return header.min_y;
  };
  inline F64 get_min_z() const {
    return header.min_z;
  };

  inline F64 get_max_x() const {
    return header.max_x;
  };
  inline F64 get_max_y() const {
    return header.max_y;
  };
  ...
  inline F64 get_y() const {
    return get_y(point.get_Y());
  };
  inline F64 get_z() const {
    return get_z(point.get_Z());
  };

  inline F64 get_x(const I32 x) const {
    return header.get_x(x);
  };
  inline F64 get_y(const I32 y) const {
    return header.get_y(y);
  };
  inline F64 get_z(const I32 z) const {
    return header.get_z(z);
  };
...
class LASLIB_DLL LASreadOpener {
 public:
  ...
  inline U32 get_io_ibuffer_size() const {
    return io_ibuffer_size;
  };
  ...
  inline BOOL is_auto_reoffset() const {
    return auto_reoffset;
  };
  inline void set_offset_adjust(const BOOL offset_adjust) {
    this->offset_adjust = offset_adjust;
  };
  inline BOOL is_offset_adjust() {
    return this->offset_adjust;
  };
  void set_files_are_flightlines(const I32 files_are_flightlines);
  inline I32 are_files_flightlines() const {
    return files_are_flightlines;
  };
  void set_files_are_flightlines_index(const I32 files_are_flightlines_index);
  inline I32 get_files_flight_index() const {
    return files_are_flightlines_index;
  };
  void set_apply_file_source_ID(const BOOL apply_file_source_ID);
  inline BOOL applying_file_source_ID() const {
    return apply_file_source_ID;
  };
  void set_scale_factor(const F64* scale_factor);
  inline const F64* get_scale_factor() const {
    return scale_factor;
  };
  ...
  void add_attribute(
      I32 data_type, const CHAR* name, const CHAR* description = 0, F64 scale = 1.0, F64 offset = 0.0, F64 pre_scale = 1.0, F64 pre_offset = 0.0,
      F64 no_data = F64_MAX);
  ...
  inline LAStransform* get_transform() {
    return transform;
  };
  void set_ignore(LASignore* ignore);
  inline LASignore* get_ignore() {
    return ignore;
  };
  void reset();
  ...
  I32 get_number_attributes() const {
    return number_attributes;
  };
  I32 get_attribute_data_type(U32 index) const {
    return attribute_data_types[index];
  };
  const CHAR* get_attribute_name(U32 index) const {
    return attribute_names[index];
  };
  const CHAR* get_attribute_description(U32 index) const {
    return attribute_descriptions[index];
  };
  F64 get_attribute_scale(U32 index) const {
    return attribute_scales[index];
  };
  F64 get_attribute_offset(U32 index) const {
    return attribute_offsets[index];
  };
  F64 get_attribute_no_data(U32 index) const {
    return attribute_no_datas[index];
  };
  void set_copc_stream_ordered_by_chunk() {
    copc_stream_order = 0;
  };
  void set_copc_stream_ordered_spatially() {
    copc_stream_order = 1;
  };
  void set_copc_stream_ordered_by_level() {
    copc_stream_order = 2;
  };
  BOOL get_use_stdin() {
    return use_stdin;
